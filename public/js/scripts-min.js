...
function animationSuccessWebsite() {
  var containerSuccess = d.getElementById('container-success');
  var coords = containerSuccess.getBoundingClientRect();
  if (d.body.scrollTop > coords.top || d.documentElement.scrollTop > coords.top) {
    var CSWebsite = d.getElementsByClassName('container-success-website');
    for (var i = 0; i < CSWebsite.length; i++) {
      CSWebsite[i].style.transform = 'translate(0)';
      CSWebsite[i].style.opacity = '1';
    }
  } else {
    var _CSWebsite = d.getElementsByClassName('container-success-website');
    for (var _i = 0; _i < _CSWebsite.length; _i++) {
      _CSWebsite[_i].style.transform = 'translate(100em)';
      _CSWebsite[_i].style.opacity = '0';
    }
  }
}
